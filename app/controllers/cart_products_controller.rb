class CartProductsController < ApplicationController
  ...
  #  Post "/addtocart" for both user and guest 
  def create
    if (current_user)
  ...
  # Delete "/cart_products/:product_id"
  def destroy
  ...
  def destroy_all
    ...
    existing_products.each do |cart_product|
      ...
      OrderedItem.create(order_id: order.id, product_id: product.id, item_quantity: cart_product[:item_quantity])
  ...
  def product_exists?
    existing_products.any? { |product| product[:product_id] == params[:product_id] }
  end
